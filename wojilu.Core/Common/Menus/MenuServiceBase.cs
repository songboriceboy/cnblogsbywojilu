...
namespace wojilu.Common.Menus {
    ...
    public abstract class MenuServiceBase : IMenuService {
        ...
        private IList GetList( int ownerId ) {
            ...
            //这里加载的是我记录SNS所有单用户模块，在framework->config->site.config->UserInitApp : home,blog,photo,microblog,friend,visitor,forumpost,about,feedback所有
            //而对于豆约翰只需要部分(主页和博客)
            string module = "";
            String[] arr = config.Instance.Site.UserQsyInitApp.Split(',');
            foreach (String app in arr)
            {
                module += "'" + app + "',";
            }
            module = module.Substring(0, module.Length - 1);
            return ndb.find(t, "Name in (" + module + ") AND OwnerId=" + ownerId + " order by OrderId desc, Id asc").list();
        }
