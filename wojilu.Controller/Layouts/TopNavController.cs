...
namespace wojilu.Web.Controller.Layouts {
    ...
    public partial class TopNavController : ControllerBase {
        ...
        public void Header() {
            ...
            set("recentLink", Link.To(new Blog.MainController().Recent));
            set("recentReadLink", Link.To(new Blog.MainController().ReadHits));
            set("searchAction", to(new Blog.MainController().Results));
            if (ctx.GetItem("searchKey") != null)
            {
                string str = ctx.GetItem("searchKey").ToString();
            }
            set("SearchKey", ctx.GetItem("searchKey") == null ? "" : ctx.GetItem("searchKey").ToString());

            set("spaceUrl", strUtil.Join(ctx.url.SiteUrl, "blogback.html"));//届时需要注册一个用户（douyuehan）
        }
