    ...
    public class RegisterController : ControllerBase {
        ...
        private void addUserAppAndMenus( User user ) {
            ...
            if (menus.Contains( "home" )) {
                new UserHomeInstaller().Install( ctx, user, "豆约翰", wojilu.Common.AppBase.AccessStatus.Public );
                IMenu menu = getMainMenu(user, "豆约翰", SystemInfo.SiteRoot);
            ...
            if (menus.Contains("blog"))
            {
                ...
                IMenu menu = menuService.AddMenuByApp(blogApp, blogApp.Name, "", blogUrl+".aspx");
                menu.Url = "default";
               // menu.OrderId = 99;
                menuService.Update(menu);
            }
            if (menus.Contains("manage"))
            {
                IMenu menu = getMenu(user, "管理", t2(new Users.Admin.UserProfileController().Profile));
                menuService.Insert(menu, user, user);
