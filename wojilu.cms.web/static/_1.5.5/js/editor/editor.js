wojilu.editorConfig = {
    ...
    basicToolbar : [ 'bold', 'forecolor', 'fontFamily', 'fontSize', 'underline', 'strikethrough', 'separator', 'link', 'emotion', 'pic' ],
    ...
    lang : {
        ...
        aboutUs : '<strong>『我记录』在线编辑器</strong><br/><a href="http://www.wojilu.com" target="_blank">www.wojilu.com</a> &copy; 1999-2010',
        sourceCode : '源代码'        
    },
...
wojilu.editor.prototype.addCallback = function () {
    ...
    // 自定义的格式化命令（带弹窗或下拉菜单的）
    var dcmds = ['table', 'fontFamily', 'fontSize', 'forecolor', 'backcolor', 'emotion', 'pic', 'flash', 'link', 'about'];
...
wojilu.editor.prototype.picDialog = function () {
    ...
    result += '		<div class="insertPannel">'+this.lang.url+':&nbsp;<input class="editorImgUrl" type="text" />&nbsp;<input class="btnInsertImg" type="button" value="'+this.lang.imgInsert+'" /></div>';
    result += '		</td>';
    result += '	</tr>';
    result += '</table>';
    result += '</div>';    
...
wojilu.editor.prototype.flashDialog = function () {
    ...
    result += '	<tr><td colspan="2" style="text-align:center;padding-bottom:20px;"><input class="btnInsertFlash" type="button" value="'+this.lang.flashInsert+'" /></td></tr>';
    result += '</table>';
    result += '</div>';
    return result;
};
...
wojilu.editor.prototype.linkDialog = function () {
    ...
    var result ='<div id="'+linkBoxId+'" class="editorBox" unselectable="on"><div><table><tr><td>'+this.lang.url+': <input class="editorLinkUrl" type="text" value="http://" />&nbsp;<input class="btnCreateLink" type="button" value="'+this.lang.addLink+'" unselectable="on" style="margin-right:10px;"/></td><td>'+this.closecmd(linkBoxId)+'</td></tr></table></div></div>';
    return result;
};
...
wojilu.editor.prototype.render = function() {
    ...
    // 弹窗中编辑器不可resize
    if( wojilu.tool.getQuery( 'frm' ) != 'true' ) {
        $('#'+this.id).resizable({
