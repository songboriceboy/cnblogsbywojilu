...
namespace wojilu.Apps.Blog.Service {
    ...
    public class BlogPostService : IBlogPostService {
        ...
        public virtual DataPage<BlogPost> GetPageAndDraft()
        {
            return db.findPage<BlogPost>("SaveStatus<=" + SaveStatus.Draft);
        }
        ...
        public virtual DataPage<BlogPost> GetPageByCreatedCate(int appId, int YmId, int pageSize)
        {
           // BlogPost blog = BlogPost.find("CategoryId=" + categoryId).list(0)[0];
            //DateTime created = blog.Created;
            DateTime created = DateTime.ParseExact(YmId.ToString(), "yyyyMM", System.Globalization.CultureInfo.CurrentCulture);
            String condition = "SaveStatus = 0 and AppId=" + appId;
           // string startdate = created.ToString("yyyy-M");
           // string enddate = created.AddMonths(1).ToString("yyyy-M");
            //if (YmId > 0) condition = condition + " and Created between #" + startdate + "# and #" + enddate + "#";
            string startdate = created.ToString("yyyyMM");
            string enddate = created.AddMonths(1).ToString("yyyyMM");
            if (YmId > 0) condition = condition + " and Created between '" + startdate + "01' and '" + enddate + "01'";
            condition = condition + " order by Created desc, Id desc";
            return db.findPage<BlogPost>(condition, pageSize);
        }
        ...
        public virtual int GetCountByCategory( int id ) {
            return db.count<BlogPost>("SaveStatus = 0 and Category.Id=" + id + " ");
        }
        public virtual int GetCountByCategory(int appId, int categoryId)
        {

            String condition = "AppId=" + appId;
            if (categoryId > 0) condition = condition + " and Category.Id=" + categoryId;
            condition = condition + " and SaveStatus=" + SaveStatus.Normal;
            return db.count<BlogPost>(condition);
        }
        public virtual int GetCountByCreated(int AppId,string start,string end)
        {
            // return db.count<BlogPost>("AppId=" + AppId + " and Created between #" + start + "# and #" + end + "#");
            return db.count<BlogPost>("SaveStatus = 0 and AppId=" + AppId + " and Created between '" + start + "01' and '" + end + "01'");
        }
        ...
        public virtual int GetBlogCount()
        {
        ...
        public virtual DataPage<BlogPost> Search(string key, int pageSize)
        {
            if (strUtil.IsNullOrEmpty(key))
                return GetPage();

            String searchStr = strUtil.SqlClean(key, 10);
            //SQL
            //return BlogPost.findPage(" (Title like '%" + searchStr + "%' or Content like '%" + searchStr + "%') and " + getNonDelCondition(), pageSize);
            //Access
            return BlogPost.findPage(" (Title like '%%" + searchStr + "%%' or Content like '%%" + searchStr + "%%') and " + getNonDelCondition(), pageSize);
        }
